...
export default function History() {
  ...
  return (
    <Stack sx={{ height: '100vh', background: (screenSize && themeContext.mode == 'light' ? 'linear-gradient(180deg, #F9FAFA 59%, #EDE4FF 100%)' : '') }} direction={'column'} justifyContent={'space-between'} >
      ...
      <Box sx={{ mb: 1, height: '100%', scrollbarWidth: 'none' }} gap={10} overflow={'scroll'} justifyContent={'end'} >
        <Typography component={'h1'} variant='p'
          sx={{ textAlign: "center", mt: chats.length ? 'auto' : '10%' }}
        >
          Conversation History</Typography>

        {chats?.length > 0 && <FilterByRating option={option} setOption={setOption} chats={chats} setFilteredChats={setFilteredChats} />}

        {filteredChats?.length > 0 ? filteredChats.map((ele, idx) =>
          <Stack
            kry={
              idx
            }
            ...
            position={'relative'}
          >
            <Typography component={'h3'} variant='p' position={'absolute'} top={-44}
              sx={{ textAlign: "center", mt: chats.length ? 'auto' : '10%' }}>
