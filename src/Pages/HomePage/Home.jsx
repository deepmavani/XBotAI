...
export default function Home() {
  ...
  console.log(context, themeContext, chats)
  ...
  return (
    <Stack sx={{ height: '100vh', background: (screenSize && themeContext.mode == 'light' ? 'linear-gradient(180deg, #F9FAFA 59%, #EDE4FF 100%)' : '') }} direction={'column'} justifyContent={'space-between'} >
      <Navbar handleSidebar={context.handleSidebar} />
      <Box ref={scrollRef} sx={{ mb: 1, height: '100%', scrollbarWidth: 'none' }} gap={10} overflow={'scroll'} display={!chats?.length > 0 && 'flex'} justifyContent={'end'} >
        {!chats?.length > 0 && <DefaultChats getAnswer={getAnswer}></DefaultChats>}
        {chats.length > 0 && chats.map((ele, idx) => <><ChatCard key={idx} details={ele?.human} setChats={setChats} /><ChatCard key={idx} details={ele?.AI} setChats={setChats} /></>)}
      </Box>
      <Input handleSave={() => {
        localStorage.setItem('chats',
          JSON.stringify([...JSON.parse(localStorage.getItem('chats')) || [], ...chats,])
        )
        setChats([])
        // navigate('/history')
      }} getAnswer={getAnswer} />
